...
namespace JBartels\mail2news\Importer;
...
class T3blog {
		...
		if(trim($categories!='')) {
			...
			foreach ($cat_array as $category) {
				...
				if (is_numeric($category)) {
					...
					$condition = 'title LIKE ' . $GLOBALS['TYPO3_DB']->fullQuoteStr($category, 'tx_t3blog_cat');
				} else {
					continue;
				}
				$where .= ' OR ' . $condition;
			...
			$where .= ' )';
			$rows = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows( 'uid', 'tx_t3blog_cat', $where, '', $orderBy, '', 'uid' );
	...
	function store_record($item, $useTceMain=false, $disableLogging=false) {
		...
		if ($useTceMain) {
			...
			$GLOBALS['TYPO3_DB']->exec_INSERTquery('tx_t3blog_post', $t3blog_post);
			$t3blog_uid = $GLOBALS['TYPO3_DB']->sql_insert_id();

		#	$tt_content['bodytext'] = $GLOBALS['TYPO3_DB']->fullQuoteStr($tt_content['bodytext'], 'tx_t3blog_cat');
			$tt_content['irre_parentid'] = $t3blog_uid;
			$GLOBALS['TYPO3_DB']->exec_INSERTquery('tt_content', $tt_content);
			$tt_content_uid = $GLOBALS['TYPO3_DB']->sql_insert_id();
	...
	#			$cat_mm_array = array();
				foreach ($categories as $category) {
					...
					$GLOBALS['TYPO3_DB']->exec_INSERTquery('tx_t3blog_post_cat_mm', $cat_mm);
					$sort++;

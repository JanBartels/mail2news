...
class Ttnews {
		...
		if(trim($categories!='')) {
			...
			foreach ($cat_array as $category) {
				...
				if (is_numeric($category)) {
					...
					$condition = 'title LIKE ' . $GLOBALS['TYPO3_DB']->fullQuoteStr($category, 'tt_news_cat');
				} else {
					continue;
				}
				$where .= ' OR ' . $condition;
	...
	function store_record($item, $useTceMain=false, $disableLogging=false) {
		...
		if ($useTceMain) {
			...
			$GLOBALS['TYPO3_DB']->exec_INSERTquery('tt_news', $tt_news);
			...
			if ($addCat) {
				...
				foreach ($categories as $category) {
					...
					$GLOBALS['TYPO3_DB']->exec_INSERTquery('tt_news_cat_mm', $cat_mm);
					$sort += 64;
				}
			}

...
namespace JBartels\mail2news\Mail;
...
class GetMail {
	...
	function process_all_importers($emConf) {
		 ...
		 */
		$emConf = is_array( $emConf ) ? array_change_key_case($emConf) : array();
		...
		$where = '1=1' . \TYPO3\CMS\Backend\Utility\BackendUtility::BEenableFields($table) . \TYPO3\CMS\Backend\Utility\BackendUtility::deleteClause($table);
		...
		if($GLOBALS['TYPO3_DB']->sql_num_rows($res)> 0) {
			...
			while (false !== ($ar = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res))) {
				...
				$mailbox_parameters = array_intersect_key($ar, array_flip(array('mail_server', 'mail_username', 'mail_password',
					'imap', 'use_ssl', 'self_signed_certificate', 'portno', 'delete_after_download', 'delete_rejected_mail')));
	...
	function getmail($conf) {
		...
		// new objects
		$imap = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('JBartels\\mail2news\\Mail\\Imap');
		...
		switch ($conf['record_type']) {
			...
			case 't3blog':
				if (\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::isLoaded('t3blog')) {
					$record = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('JBartels\\mail2news\\Importer\\T3blog');
			...
			default:
				if ( \TYPO3\CMS\Core\Utility\ExtensionManagementUtility::isLoaded('tt_news') ) {
					$record = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('JBartels\\mail2news\\Importer\\Ttnews');
				}
		}
	...
	function getTSConfig($id) {
		$rootLineStruct = \TYPO3\CMS\Backend\Utility\BackendUtility::BEgetRootLine($id);
		// get TSconfig
		$pagesTSC = \TYPO3\CMS\Backend\Utility\BackendUtility::getPagesTSconfig($id, $rootLineStruct);
	...
	*/
	function saveattachment($filename, $fileext, $savepath, $attachment, $maxsize, &$filelist, &$counter) {
		...
		if (strlen($attachment) <= $maxsize * 1024) {
			...
			$absfilename = \TYPO3\CMS\Core\Utility\GeneralUtility::getFileAbsFileName($savepath . $savedfilename);
			$i = 1;
			...
			if (!\TYPO3\CMS\Core\Utility\GeneralUtility::writeFile($absfilename,$attachment)) {
